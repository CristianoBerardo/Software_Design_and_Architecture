...
\paragraph{Request-response} - A client makes a request to a service and waits
for a response. The client blocks while waiting.

\paragraph{Asynchronous request/response} - A client sends a request to a
service, which replies asynchronously. The client doesn't block while waiting.

\paragraph{One-way notifications} - A client sends a request to a service, but no
reply is expected or sent.


\subsection{One-to-many communication}

\paragraph{Publish/subscribe} - A client publishes a notification message, which
is consumed by zero or more interested services.

\paragraph{Publish/async. response} - A client publishes a request message and
...
\paragraph{Header}
The header contains metadata about the message, such as:

\begin{itemize}
  \item Message ID
  \item Return address
  \item Metadata that describes the content of the message
  \item Collection of key-value pairs useful for routing, filtering, etc.
\end{itemize}
...
\paragraph{Command} - This is equivalent of an RPC request. It specifies the
operation to invoke and its parameters.

\paragraph{Document} - Contains only data. The receiver decides how to interpret
it, e.g. a reply to a command

\paragraph{Event} -  Indicates that something notable has occurred. An event is
often a domain event, which represents a state change of a domain object.


\section{Channel}
Abstraction of the mechanism to transfer
messages.
...
\begin{center}
  \begin{minipage}{0.48\linewidth}
    \centering
    \includegraphics[width=\linewidth]{content/chapters/images/2026-04-11-10-58-16.png}
  \end{minipage}%
  \hfill
  \begin{minipage}{0.48\linewidth}
    \centering
    \includegraphics[width=\linewidth]{content/chapters/images/2026-04-11-10-59-30.png}
  \end{minipage}
\end{center}
...
\begin{center}
  \begin{minipage}{0.45\linewidth}
    \centering
    \includegraphics[width=\linewidth]{content/chapters/images/2026-04-11-11-20-19.png}
  \end{minipage}%
  \hfill
  \begin{minipage}{0.45\linewidth}
    \centering
    \includegraphics[width=\linewidth]{content/chapters/images/2026-04-11-11-20-44.png}
  \end{minipage}
\end{center}
