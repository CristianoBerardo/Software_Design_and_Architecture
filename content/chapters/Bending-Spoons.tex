...
\section{Bending Spoons seminary (Evernote)}
...
%   \begin{tabular}{l l}
%     \toprule
%     \textbf{They look for:} & \textbf{The processes:} \\
%     \midrule
%     Drive                   & Screening               \\
%     Potential               & Tasks                   \\
%     Passion                 & Interviews              \\
...
\textbf{Legacy}: "Monolith have you some new content?" "Yes, the note with the monkey"
This is a polling request useless for the user and for the backend system.


\textbf{Microservices}: Persistent connection between the client and the server, faster
...
Middle consulting company in Italy.

\subsection{Cloud core banking}

Migrate the big italian bank to new cloud architecture, the new digital bank.


\subsection{VAULT}

An API and event driven platform tgat provides default capabilities 
to reduce time to market by semplifing inregration.

\subsection{How build building new brand on top traditional bank}

Develop new digital bank like IsyBank (Vault + other services).
Thre main steps:

% \begin{itemize}
%   \item Abstraction layer
%   \item Dual run
%   \item Migration
% \end{itemize}
...
Middlerware that provide abstraction between the client and the backend providing
the solution to migrate the old system to the new one without affecting the client experience.
